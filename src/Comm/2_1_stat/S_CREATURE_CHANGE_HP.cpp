...
void* S_CREATURE_CHANGE_HP_f(const void** argv)
{
	...
	int*	type = (int*)argv[4];
	packet* packet_l = new_packet(S_CREATURE_CHANGE_HP, 32);

	packet_l->write_4(player_l->Stats.HP); // сколько сейчас
	packet_l->write_4(player_l->Stats.maxHP[1]); // сколько размер полоски
	...
	if(attacker1 || attacker2) {
		packet_l->write_4(attacker1 ? attacker1->Id : attacker2->Id); // ИД того кто вас хильнул
